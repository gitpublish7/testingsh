...
TOKEN = '7947753994:AAGtsgrHxo6OeLBeq9-EdZM7u08oTNdFQ6w'
...
def fetch_proxy_list_from_free_proxy_api(api_url="https://www.freeproxylists.net/api/proxies.json?protocol=https"):
    try:
        response = requests.get(api_url)
        if response.status_code == 200:
            proxy_data = response.json()  # JSON response
            proxy_list = [proxy['ip'] + ':' + str(proxy['port']) for proxy in proxy_data]
            return proxy_list
        else:
            logging.error(f"Failed to fetch proxy list from Free Proxy API. Status code: {response.status_code}")
            return []
...
def update_proxy():
    proxy_list = fetch_proxy_list_from_free_proxy_api()  # Fetch proxies from Free Proxy API
    if proxy_list:
        proxy = random.choice(proxy_list)  # Select a random proxy from the list
        telebot.apihelper.proxy = {'https': proxy}
        logging.info("Proxy updated successfully.")
    else:
        logging.error("Proxy list is empty. Proxy not updated.")
